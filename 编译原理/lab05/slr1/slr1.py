...
def create_slr1():
    ...
    for i in range(len(status_trans)):
        if status_trans[i].x in VT:
            SLR_TABLE[status_trans[i].status_init][VT.index(status_trans[i].x)] = 's' + str(status_trans[i].status_trans)
        else:
            SLR_TABLE[status_trans[i].status_init][VN.index(status_trans[i].x) + len(VT) - 1] = status_trans[i].status_trans
